...
int main(){
	...
	for(int j = 0; j < s.size(); j++){
		g[1].push_back({s[j] - 'A' + 2, j, 1});
	}

	for(int i = 0; i < n-1; i++){
		for(int j = 0; j < strs[i].size(); j++){
			for(int k = 0; k < strs[i+1].size(); k++){
				int u = strs[i][j] - 'A' + 2 + i;
				int v = strs[i+1][k] - 'A' + 2 + i + 1;
