...
constexpr int L = ceil(log(N)); // L = ceil(log(N))
...
/* O(V) - Fills parent[], weight[], sizer[] and depth[]. */
...
void hld_init(int root) {
    ...
    for (int u = 1; u <= n; u++) {
        for (int i = 1; i < g[u].size(); i++) {
            if (sizer[g[u][i].first] > sizer[g[u][0].first]) {
                swap(g[u][i], g[u][0]);
            }
        }
    }
...
void solve(){
	...
	while(s != "DONE"){
		if(s == "QUERY"){
			int u, v;
			cin >> u >> v;
			cout << hld_query(u, v) << endl;
		}
		else{
			int i, ti;
			cin >> i >> ti;
			hld_update(edgs[i].fi, edgs[i].se, ti);
		}
