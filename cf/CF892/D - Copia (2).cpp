...
void solve(){
	...
	for(int i = 0; i < n; i++){
		...
		if(b.empty()){
			...
			int xl = (int)(lower_bound(x.begin(), x.end(), make_pair(lb[s],-1)) - x.begin());
			int xr = (int)(upper_bound(x.begin(), x.end(), make_pair(rb[s],-1)) - x.begin());
			...
			int xl = (int)(lower_bound(x.begin(), x.end(), make_pair(cur_lb,-1)) - x.begin());
			int xr = (int)(upper_bound(x.begin(), x.end(), make_pair(cur_rb,-1)) - x.begin());

			update(1, 1, q, xl+1, xr+1, cur_rv);
